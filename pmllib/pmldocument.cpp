...
string PMLAlgorithms::itoa( int i ){
    std::stringstream ss;
    ss <<  i;
    return ss.str();
}
...
string PMLAlgorithms::ftoa( double f ){
    std::stringstream ss;
    ss <<  f;
    return ss.str();
}
...
// Iterate through MEASURE contents
	...
	// NOTE
		...
		//this is the original bit of code before I added the rests - brought back in because mystuff above doesn't work
			 ...
			 This is the code I added but have taken out because labelling the rests and ties separately doesn't work with db
